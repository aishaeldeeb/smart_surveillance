...
parser.add_argument('--anomaly_dir', default='/scratch/aishaeld/I3D_Feature_Extraction_resnet/features_v2/anomaly')
parser.add_argument('--non_anomaly_dir', default='/scratch/aishaeld/I3D_Feature_Extraction_resnet/features_v2/non_anomaly')
parser.add_argument('--train_file', default='train_list_v2.list')
...
with open(test_file, 'w') as f:
    for i in range(anomaly_train_count, len(anomaly_files)):
        f.write(os.path.join(anomaly_dir, anomaly_files[i]) + '\n')
    for i in range(non_anomaly_train_count, len(non_anomaly_files)):
        f.write(os.path.join(non_anomaly_dir, non_anomaly_files[i]) + '\n')

print(f'Number of anomaly files: {len(anomaly_files)}\n')
print(f'Number of non-anomaly files: {len(non_anomaly_files)}\n')
